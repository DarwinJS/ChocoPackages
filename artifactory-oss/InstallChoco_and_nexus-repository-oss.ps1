...
$Description = "Artifactory OSS install on $env:Computername"
If ($host.name -ilike "*remote*") {$Description = "Artifactory OSS on `$env:Computername"}
$Changes = @"
  [1] Sets PowerShell Execution Policy to "RemoteSigned"
  [2] Installs chocolatey package manager
  [3] Installs the chocolatey package artifactory-oss
"@
