...
as they work on a pristine machine no matter whether it is domain joined or not.
  ...
  2) Paste this command into the console (get the whole line - it's long):
     [System.Net.ServicePointManager]::ServerCertificateValidationCallback = {[bool]1};set-executionpolicy RemoteSigned -Force -EA 'SilentlyContinue';iex ((new-object net.webclient).DownloadString('https://raw.githubusercontent.com/DarwinJS/ChocoPackages/master/artifactory-pro/InstallChoco_and_artifactory-pro.ps1'))
#>

$DoNotPrompt = $true

$Description = "Artifactory Pro install on $env:Computername"
If ($host.name -ilike "*remote*") {$Description = "Artifactory Pro on `$env:Computername"}
$Changes = @"
  [1] Sets PowerShell Execution Policy to "RemoteSigned"
  [2] Installs chocolatey package manager
  [3] Installs the chocolatey package artifactory-pro
"@
...
choco install artifactory-pro -confirm
