...
.EXAMPLE
    # Get-Counter is built into PowerShell
    
    Get-counter -listset * | select countersetname
    
    Lists all possible counter sets you can monitor
.EXAMPLE
    # Get-Counter is built into PowerShell
    
    Get-Counter -listset memory | select -expand paths
    
    Lists exactly the counter paths you would use to monitor them with this script
