...
Else
{
  If ($Win7)
  {
    If (-not([bool](get-hotfix | where {$_.hotfixId -eq 'KB3065987'})))
    {
      ...
      Install-ChocolateyPackage "$packageName" 'MSU' -url $url7 -url64 $url764 -checksum $checksum7 -checksumtype 'sha1' -checksum64 $checksum764 -checksumtype64 'sha1'
    }
  }
  If ($Win81)
  {
    ...
    {
      Write-Output "Applying required hotfix KB3065987"
      Install-ChocolateyPackage "$packageName" 'MSU' -url $url81 -url64 $url8164 -checksum $checksum81 -checksumtype 'sha1' -checksum64 $checksum8164 -checksumtype64 'sha1'
    }
  }
  ...
  $REGKey2 = 'HKLM\Software\Policies\Microsoft\Windows\Gwx'
